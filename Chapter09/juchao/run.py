...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    """
    signal_status = Signal(str, list)  # 自定义的信号，用来显示状态栏。
    ...
    def show_tablewidget(self, dict_data, tableWidget, clear_fore=True):
        ...
        if len(dict_data) > 0:
            ...
            for index in range(len_index):
                ...
                '提取url'
                download_url = 'http://static.cninfo.com.cn/' + dict_temp['adjunctUrl']
            ...
            df = self.filter_df(df, filter_title_list=self.filter_title_list,
    ...
    def update_tablewidget_title(self, page_num=1):
        ...
        if total_pages_title != None:
            ...
            self.label_page_info_title.setText(
                '%d/%d' % (self.current_page_num_title, self.total_pages_title))  # 更新当前页码信息
    ...
    def on_pushButton_search_clicked(self):
        ...
        if self.end_time < self.start_time:
            reply = QMessageBox.information(self, "日期出错", "开始日期%s>截止日期%s，请重选择" % (self.start_time, self.end_time),
